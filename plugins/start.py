...
async def handle_start(bot: Client, message: Message):
    ...
    if Config.START_PIC:
        if message.from_user.id == Config.ADMIN:
            ...
            await SnowDev.delete()
            await message.reply_photo(photo=Config.START_PIC, caption=text, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="ᴅᴇᴠᴇʟᴏᴘᴇʀ 👨‍💻", url="https://t.me/")]]))
    else:
        if message.from_user.id == Config.ADMIN:
            await SnowDev.edit(text=text, reply_markup=reply_markup)
        else:
            await SnowDev.edit(text=text, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="ᴅᴇᴠᴇʟᴏᴘᴇʀ 👨‍💻", url="https://t.me/")]]))
